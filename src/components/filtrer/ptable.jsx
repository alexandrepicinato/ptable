...
export default function TableFiltred(){
    ...
    function FiltredClass(number ,filttd){
        var familia =FamilyByPosition(number)
        console.log(familia+"/"+ FiltrerDataid)
        if(familia == FiltrerDataid){
            return "flex"
        }
        else{
            return "none"
            ...
            flexDirection:"column",
                ...
                <div>
                    <div style={{
                        ...
                        <h1>Filtrado Por {FiltrerDataid}</h1>
                        {
                            dataJson.rows.map(
                                (elementInfo)=>
                                <>
                                                ...
                                                elementInfo.data.map(
                                                    (data) => (
                                                        ...
                                                        <PTable_Element className={FamilyByPosition(data.natomico)} data-element={data.nome} data-simbolo={data.simbolo} data-massaatomica={data.massaatomica} data-numeroatomico={data.natomico} data-fusao={data.fusao} data-ebulicao={data.ebulicao} data-densidade={data.densidade} data-distroeletronica={data.eletronicconfig} data-data-distribuicaoatomica={distribuicaoatomica(data.natomico)} data-familia={FamilyByPosition(data.natomico)} naa={data.natomico} style={{
                                                            display:FiltredClass(data.natomico,FiltrerDataid)
                                                        }}>
                                                            <div>
